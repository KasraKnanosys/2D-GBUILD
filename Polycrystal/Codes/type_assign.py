...
import sys
if len(sys.argv) != 2:
    print("Syntax: python type_assign.py GB_Name.lmp")
    sys.exit()
...
with open('min.lmp','r') as fdata:
    ...
    dim_x = float(lines[5].split()[1]) - float(lines[5].split()[0])
    dim_y = float(lines[6].split()[1]) - float(lines[6].split()[0])
    dim_z = float(lines[7].split()[1])
    #print(dim_x,dim_y,dim_z)
    for i in range(16,n_atom+16):
        #print(i)
        data = lines[i].split()
...
with open('bond_data_all.txt','r') as fdata:
    for line in fdata:
        data=line.split()
        #print(data)
...
with open('same_atom_all.txt','r') as fdata:
    for line in fdata:
        data=line.split()
        #print(data)
...
with open('hBN.lmp','w') as fdata:
    fdata.write('# LAMMPS data file written by python_NSAKIB\n')
    fdata.write('{} {}\n'.format(n_atom,'atoms'))
    fdata.write('2 atom types\n')
    fdata.write('{} {} {}\n'.format(-dim_x/2,dim_x/2,'xlo xhi'))
    fdata.write('{} {} {}\n'.format(-dim_y/2,dim_y/2,'ylo yhi'))
    fdata.write('{} {} {}\n'.format('0.0',dim_z,'zlo zhi'))
    fdata.write('\n')
    fdata.write('Masses\n')
    fdata.write('\n')
    fdata.write('1  10.81000000     # B\n')
    fdata.write('2  14.00670000     # N\n')
    fdata.write('\n')
...
with open('WSe2_'+sys.argv[1][2:],'w') as fdata:
    fdata.write('# LAMMPS data file written by python_NSAKIB\n')
    fdata.write('{} {}\n'.format(len(convert_wse2),'atoms'))
    fdata.write('2 atom types\n')
    fdata.write('{} {} {}\n'.format(-(dim_x*conv_multi)/2,(dim_x*conv_multi)/2,'xlo xhi'))
    fdata.write('{} {} {}\n'.format(-(dim_y*conv_multi)/2,(dim_y*conv_multi)/2,'ylo yhi'))
